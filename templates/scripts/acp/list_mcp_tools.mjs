...
function main() {
  ...
  try {
    ...
    for (const item of output) {
      console.log(
        `${item.tool_id} type=${item.tool_type || "-"} runtime=${item.runtime_type || "-"} capability=${item.capability} npm_package=${item.npm_package || "-"} mode=${item.route_mode} handler=${item.route_handler}`,
      );
    }
  } catch (error) {
